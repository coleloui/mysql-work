...
SELECT author_lname, released_year, title FROM books ORDER BY author_lname DESC, released_year;
...
SELECT title, CONCAT(author_fname, ' ', author_lname) AS author FROM books WHERE author_fname LIKE '%da%';

SELECT * FROM books WHERE title LIKE '%:%';

SELECT * FROM books WHERE author_fname LIKE '____';

DELETE FROM books WHERE title IN ( SELECT title from books GROUP BY title HAVING COUNT(*) > 1 );
